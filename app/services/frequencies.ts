...
export async function getFrequencyCategories(): Promise<string[]> {
    try {
        const { data } = await api.get('/folders');
        return data.folders.map((folder: { name: string }) => folder.name);
    } catch (error) {
...
export async function getFrequenciesByCategory(categoryId: string): Promise<FrequenciesResponse> {
    try {
        const encodedCategoryId = encodeURIComponent(categoryId.trim());
