...
export default function FoodManagement() {
    const [foods, setFoods] = useState<Card[]>([]);
    ...
    const fetchFoods = async () => {
        try {
            const res = await fetch('/api/admin/cards?category=FOOD');
    ...
    const openEdit = (card: Card) => {
        ...
        setFormData({
            namePt: card.namePt,
            slug: card.slug || card.namePt.toLowerCase().replace(/\s+/g, '-'),
            imageUrl: card.imageUrl,
            audioPt: card.audioPt || '',
            bgColor: card.bgColor || card.color || '#ffffff',
        });
        setIsModalOpen(true);
    };
            ...
            <div style={{ backgroundColor: '#fff', borderBottom: '1px solid #e0e0e0', padding: '1.5rem 2rem' }}>
                <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <h1 style={{ fontSize: '1.75rem', color: '#2c3e50', margin: 0 }}>🍎 Gerenciar Alimentos</h1>
                        <button
            ...
            <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '2rem' }}>
                {loading ? (
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1.5rem' }}>
                        {foods.length === 0 && <p>Nenhum alimento cadastrado.</p>}
                        {foods.map((card) => (
                            <div key={card.id} style={{ backgroundColor: '#fff', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
                                <div style={{ height: '150px', backgroundColor: card.bgColor || '#eee', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                    {card.imageUrl ? (
            ...
            {isModalOpen && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                    <div style={{ backgroundColor: '#fff', padding: '2rem', borderRadius: '12px', width: '100%', maxWidth: '500px' }}>
                        <h2 style={{ marginTop: 0 }}>{editingCard ? 'Editar Alimento' : 'Novo Alimento'}</h2>
                        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            ...
                            <div>
                                <label style={{ display: 'block', marginBottom: '0.5rem' }}>Cor de Fundo do Card (Hex)</label>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={async (e) => {
                                        ...
                                        const uploadData = new FormData();
                                        uploadData.append('file', file);
                                        uploadData.append('folder', 'audio/food'); // Pasta específica
