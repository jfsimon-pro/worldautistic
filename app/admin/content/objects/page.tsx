...
export default function ObjectsManagement() {
    const [objects, setObjects] = useState<Card[]>([]);
    ...
    const fetchObjects = async () => {
        try {
            const res = await fetch('/api/admin/cards?category=OBJECTS');
            ...
            <div style={{ backgroundColor: '#fff', borderBottom: '1px solid #e0e0e0', padding: '1.5rem 2rem' }}>
                <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <h1 style={{ fontSize: '1.75rem', color: '#2c3e50', margin: 0 }}>⚽ Gerenciar Objetos</h1>
                        <button
            ...
            <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '2rem' }}>
                {loading ? (
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1.5rem' }}>
                        {objects.length === 0 && <p>Nenhum objeto cadastrado.</p>}
                        {objects.map((card) => (
            ...
            {isModalOpen && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                    <div style={{ backgroundColor: '#fff', padding: '2rem', borderRadius: '12px', width: '100%', maxWidth: '500px' }}>
                        <h2 style={{ marginTop: 0 }}>{editingCard ? 'Editar Objeto' : 'Novo Objeto'}</h2>
                                ...
                                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                                    <input
                                        ...
                                        onChange={async (e) => {
                                            ...
                                            uploadData.append('folder', 'objects');
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={async (e) => {
                                        ...
                                        const uploadData = new FormData();
                                        uploadData.append('file', file);
                                        uploadData.append('folder', 'audio/objects'); // Pasta específica
